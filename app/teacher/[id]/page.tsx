      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 lg:gap-4 flex-wrap">
        <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 flex-wrap">
          <ReviewStars count={Number(AvgRatings({
            ...
            lesson_preparedness: review.lesson_preparedness,
            school_cleanliness: review.school_cleanliness,
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 flex flex-col gap-2 sm:gap-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                <UserIcon />
                <span className="text-[#0F1724] font-[Outfit] text-sm sm:text-base font-medium leading-5">Would you return for this teacher?</span>
              </div>
            ...
            <div className="flex gap-2 sm:gap-2.5 pl-4 sm:pl-5 items-center">
              ...
              <p className="text-[#464555] font-inter text-sm sm:text-[15px] italic font-normal leading-5">{review.teacher_comment || "No Comment"}</p>
            </div>
          </div>
        </div>
      </div>
...
export default function TeacherDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      try {
        ...
        const response = await fetch(`/api/view-teacher/${teacherId}/reports?${params}`);
        if (!response.ok) {
  ...
  }, [activeFilter, currentPage, teacherId, hasReportAccess]);
  ...
  useEffect(() => {
    ...
    const fetchTeacherData = async () => {
      try {
        ...
        const response = await fetch(`/api/view-teacher/${teacherId}`);
        if (!response.ok) {
          throw new Error("Failed to fetch teacher data");
        }
        const data: ApiResponse = await response.json();
      ...
      <div className="min-h-screen bg-[#F8FAFE] flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className="text-red-600 font-inter text-lg">{error || "Teacher not found"}</div>
          </div>
        </main>
        <Footer />
      </div>
    ...
    <div className="min-h-screen bg-[#F8FAFE] flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-14 py-6 sm:py-10 pb-12 sm:pb-[80px]">
        <div className="flex items-center justify-between gap-3 mb-4">
          ...
          <Link
            href={`/submit-report?schoolId=${schoolId}&teacherId=${params.id}`}
        ...
        <div className="bg-white rounded-xl shadow-[0_4px_6px_-1px_rgba(0,0,0,0.10)] p-4 sm:p-6 lg:p-10 mb-6 sm:mb-10">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 sm:gap-6">
            {/* Left: name, location, school, subject */}
            <div className="flex flex-col gap-2 sm:gap-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                <div className="flex gap-[7px] items-center">
                  <SchoolIcon />
                  <Link href={`/school/${teacherData.school_id}`} className="text-[#414141] text-sm font-normal leading-[20px] break-words">
                    {teacherData.schools.school_name}
                  </Link>
                </div>
                {/* {teacherData.schools?.school_name && (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 lg:gap-7 flex-wrap">
            <div className="flex flex-col gap-1 sm:gap-2">
              ...
              <div className="flex items-center gap-1 sm:gap-1.5">
                <FileIcon />
                <span className="text-[#191C1D] font-[Outfit] text-lg sm:text-xl font-semibold">{teacherData.total_reports || 0}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1 sm:gap-2">
              <span className="text-[#AFAFB2] font-[Outfit] text-sm sm:text-base font-medium leading-6 sm:leading-7">Would Return to Teacher</span>
              <div className="flex items-center gap-3 sm:gap-4 lg:gap-5 flex-wrap">
                <div className="flex items-center gap-1 sm:gap-1.5">
                  <span className="text-[#10B981] font-[Outfit] text-lg sm:text-xl font-semibold">{teacherData.return_to_teacher_yes_percentage}%</span>
                  <span className="text-[#10B981] font-[Outfit] text-lg sm:text-xl font-semibold">Yes</span>
                </div>
                <div className="flex items-center gap-1 sm:gap-1.5">
                  <span className="text-[#E8A411] font-[Outfit] text-lg sm:text-xl font-semibold">{teacherData.return_to_teacher_maybe_percentage}%</span>
                  <span className="text-[#E8A411] font-[Outfit] text-lg sm:text-xl font-semibold">Maybe</span>
                </div>
                <div className="flex items-center gap-1 sm:gap-1.5">
                  <span className="text-[#E02C2C] font-[Outfit] text-lg sm:text-xl font-semibold">{teacherData.return_to_teacher_no_percentage}%</span>
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
            <h2 className="text-[#121212] font-[Outfit] text-2xl sm:text-3xl font-semibold leading-none">Recent Reports</h2>

            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              {[
                { label: "All", count: teacherData.total_reviews },
                { label: "Positive", count: teacherData.positive_reports },
                { label: "Neutral", count: teacherData.neutral_reports },
                { label: "Negative", count: teacherData.negative_reports },
              ].map(({ label, count }) => (
                <button
                  key={label}
                ...
                {sessionStatus !== "authenticated" ? (
                  <>
                    <p className="text-[#6B7280] text-sm sm:text-base max-w-md leading-relaxed mb-4">
                      Detailed teacher reports are only available to approved Guest Teachers. Please log in to request access.
                    </p>
                    <Link
                      href="/login"
                  ...
                  <p className="text-[#6B7280] text-sm sm:text-base max-w-md leading-relaxed">
                    Detailed teacher reports are only available to approved Guest Teachers.
                  </p>
                ) : accessStatus === "pending" ? (
                  <p className="text-[#E8A411] text-sm sm:text-base max-w-md leading-relaxed font-medium">
                    Your access request for {teacherData.schools?.school_name || "this teacher's school"} is pending Super Admin approval.
                  </p>
                ) : (
                  <>
                    <p className="text-[#6B7280] text-sm sm:text-base max-w-md leading-relaxed mb-4">
                      {accessStatus === "rejected"
                        ? `Your previous access request for ${teacherData.schools?.school_name || "this teacher's school"} was rejected. You can request access again.`
                        : accessStatus === "revoked"
                        ? `Your access to ${teacherData.schools?.school_name || "this teacher's school"}'s detailed reports has been revoked. You can request access again.`
                        : `Request access from the Super Admin to view detailed reports for ${teacherData.schools?.school_name || "this teacher's school"}.`}
                    </p>
                    <button
