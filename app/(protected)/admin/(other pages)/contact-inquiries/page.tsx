        ...
        <div className="overflow-hidden rounded-xl border border-black/10 bg-white">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-left">
              ...
              <tbody>{inquiries.map((inquiry) => <tr key={inquiry.id} className="border-b border-black/5 last:border-0"><td className="px-5 py-4"><p className="font-inter text-sm font-semibold text-[#121212]">{inquiry.full_name}</p><p className="font-inter text-xs text-[#737786]">{inquiry.email}</p></td><td className="max-w-[260px] truncate px-5 py-4 font-inter text-sm text-[#30343B]">{inquiry.subject}</td><td className="px-5 py-4 font-inter text-sm text-[#737786]">{new Date(inquiry.created_at).toLocaleDateString()}</td><td className="px-5 py-4"><select aria-label={`Status for ${inquiry.subject}`} value={inquiry.status} onChange={(event) => updateStatus(inquiry.id, event.target.value as InquiryStatus)} className="rounded-lg border border-[#D9DEE7] bg-white px-3 py-2 font-inter text-sm text-[#30343B] outline-none focus:border-[#0171F9]"><option value="new">New</option><option value="read">Read</option><option value="resolved">Resolved</option></select></td><td className="px-5 py-4 text-right"><button onClick={() => setSelectedInquiry(inquiry)} className="font-inter text-sm font-semibold text-[#0171F9] hover:text-blue-700">View</button></td></tr>)}</tbody>
            </table>
          </div>
        </div>
      )}

      {selectedInquiry && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setSelectedInquiry(null)}><section role="dialog" aria-modal="true" aria-labelledby="inquiry-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl sm:p-8" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="font-inter text-xs font-semibold uppercase tracking-wide text-[#0171F9]">{statusLabels[selectedInquiry.status]}</p><h2 id="inquiry-title" className="mt-1 font-outfit text-2xl font-semibold text-[#121212]">{selectedInquiry.subject}</h2></div><button aria-label="Close inquiry" onClick={() => setSelectedInquiry(null)} className="text-2xl leading-none text-[#737786]">&times;</button></div><div className="mt-6 grid gap-1 font-inter text-sm text-[#5F6673]"><span className="font-semibold text-[#212121]">{selectedInquiry.full_name}</span><a href={`mailto:${selectedInquiry.email}`} className="text-[#0171F9]">{selectedInquiry.email}</a></div><p className="mt-6 whitespace-pre-wrap font-inter text-base leading-7 text-[#30343B]">{selectedInquiry.message}</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={() => updateStatus(selectedInquiry.id, "read")} className="rounded-lg border border-[#D9DEE7] px-4 py-2 font-inter text-sm font-semibold text-[#30343B]">Mark Read</button><button onClick={() => updateStatus(selectedInquiry.id, "resolved")} className="rounded-lg bg-[#0171F9] px-4 py-2 font-inter text-sm font-semibold text-white">Mark Resolved</button></div></section></div>}
    </main>
  );
}
